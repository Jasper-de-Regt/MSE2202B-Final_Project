...
void followWallReverse(int ci_drive_speed, char wallSide, int desiredDistance) {
  ...
  if ((wallSide == 'R') || (wallSide == 'r')) {
    ...
    if (((frontRightSensorData + backRightSensorData) / 2) == desiredDistance) {    // if the correct distance away from wall
      if (frontRightSensorData == backRightSensorData) {                            // if correct distance from wall, and driving parallel, drive straight
        driveStraightReverse(ci_drive_speed);
      }
      if (frontRightSensorData > backRightSensorData) {                             // if correct distance from wall, and driving towards wall, turn away from wall a little
        turnRight(ci_drive_speed, speedModifier);
      }
...
}//****************end of followWallReverse****************end of followWallReverse****************
...
//*******************************************************************************
 ...
 /* unsigned long prevmillis = 0;
   ...
   const long move_forward_time = 5000;
   const long move_back_time = 5000;

   int back_or_forward = 0;
   while (magnet_found == false) {
     ...
     if (back_or_forward == 0) {
       ...
       while (((encoder_rightMotor_raw < ci_mode2_movement_backward) || (encoder_leftMotor_raw < ci_mode2_movement_backward)) && (magnet_found == false)) {
       //while (curmillis - prevmillis <= move_forward_time && (magnet_found == false)) {
        // curmillis = millis();
         hall_effect_raw = analogRead(3);

         followWall (1600, 'R', ci_mode2_wall_distance);
     ...
     if (back_or_forward == 1) {
       ...
       while (((encoder_rightMotor_raw < ci_mode2_movement_forward) || (encoder_leftMotor_raw < ci_mode2_movement_forward)) && (magnet_found == false)) {
       //while (curmillis - prevmillis <= move_forward_time && (magnet_found == false)) {
