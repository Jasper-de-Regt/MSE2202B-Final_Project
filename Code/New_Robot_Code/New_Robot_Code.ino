...
const int ci_turntable_left_position = 400;      // encoder ticks of the turntable at far left position
const int ci_turntable_center_position = 980;    // encoder ticks of the turntable at center position (straight forward)
const int ci_turntable_right_position = 1540;    // encoder ticks of the turntable at far right position
...
const int ci_arm_carry_height = 200;      // encoder ticks with the arm at height ideal for driving around and not blocking the front ping sensor
...
const int ci_wrist_position_parallel = 100;  //  " Wrist bar is parallel to the arm.
...
void setup() {
  ...
  encoder_turntable_motor.zero();       // Robot arm must be positioned on the standoffs during powerup to be properly zeroed
  encoder_arm_motor.zero();             // pretty sure encoders start out at 0 anyway and these calls are redundant
